...
if __name__ == "__main__":
    ...
    for filename in list(video_params):
        ...
        while True:
            ...
            #     frame = cv2.circle(frame, (int(corrected_p1[i, 0, 0]), int(corrected_p1[i, 0, 1])), 5, (0, 0, 255), 2)
            #
            # try:
        ...
        if save_npy:
            np.save(base_path + 'tracked_data/' + basename, trace)
            # np.save(base_path + 'tracked_data/' + basename + "_centered", centered_trace)
        del trace
        # del centered_trace
