...
if __name__=="__main__":
    ...
    df, metadata = cvU.h5load("output_latest/crawling_pca.h5")
    print(df.head())
    print(df.describe())

    X_data = df.iloc[:,:5].values

    y_data = df.leech_pred.values.astype(int)
    y_data_labeled = label_binarize(y_data, classes=np.unique(y_data))
    # y_data = OneHotEncoder().fit_transform(y_data)



    X_train, X_test, y_train, y_test, y_train_labeled, y_test_labeled = train_test_split(X_data, y_data, y_data_labeled)

    classifier = LogisticRegression(max_iter=400, multi_class='ovr', n_jobs=1)
